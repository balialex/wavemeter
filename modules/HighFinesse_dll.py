...
# Modifications by Wei Wu, 2025.11
...
class HFDLL:
    def __init__(self):
        # load dll
        try:
            self.lib = cdll.LoadLibrary("C:/Windows/System32/wlmData.dll")
        except FileNotFoundError:
            raise Exception("\"wlmData.dll\" not found. Is the wavemeter software installed correctly?")

        # binds with CORRECT parameter order based on original code
        ...
        # CORRECTED: Fixed parameter order based on original working code
        self.GetActiveChannel = bind(self.lib, "GetActiveChannel", 
                                    [c_long, POINTER(c_long), c_long], c_long)
        self.SetActiveChannel = bind(self.lib, "SetActiveChannel", 
                                    [c_long, c_long, c_long, c_long], c_long)
        
        # Direct channel reading functions (correct)
        self.GetFrequency = bind(self.lib, "GetFrequency", [c_double], c_double)
        self.GetFrequency2 = bind(self.lib, "GetFrequency2", [c_double], c_double)
        self.GetWavelength = bind(self.lib, "GetWavelength", [c_double], c_double)
        self.GetWavelength2 = bind(self.lib, "GetWavelength2", [c_double], c_double)

        # Additional functions for completeness
        self.GetTemperature = bind(self.lib, "GetTemperature", [c_double], c_double)
        self.GetPressure = bind(self.lib, "GetPressure", [c_double], c_double)
        self.GetDistance = bind(self.lib, "GetDistance", [c_double], c_double)
        self.GetSwitcherChannel = bind(self.lib, "GetSwitcherChannel", [c_long], c_long)
        self.SetSwitcherChannel = bind(self.lib, "SetSwitcherChannel", [c_long], c_long)
        self.GetWLMVersion = bind(self.lib, "GetWLMVersion", [c_long], c_long)
        self.GetWLMIndex = bind(self.lib, "GetWLMIndex", [c_long], c_long)
        self.GetWLMCount = bind(self.lib, "GetWLMCount", [c_long], c_long)
        self.GetChannelsCount = bind(self.lib, "GetChannelsCount", [c_long], c_long)
        self.GetAmplitudeNum = bind(self.lib, "GetAmplitudeNum", [c_long, c_long, c_long], c_long)
        self.GetIntensityNum = bind(self.lib, "GetIntensityNum", [c_long, c_long], c_double)
        self.GetPowerNum = bind(self.lib, "GetPowerNum", [c_long, c_long], c_double)
        self.GetLinewidthNum = bind(self.lib, "GetLinewidthNum", [c_long, c_long], c_double)
        self.GetOperationState = bind(self.lib, "GetOperationState", [c_ushort], c_ushort)
        self.GetAutoCalMode = bind(self.lib, "GetAutoCalMode", [c_long], c_long)
        self.SetAutoCalMode = bind(self.lib, "SetAutoCalMode", [c_long], c_long)

        # constants from wlmData.h (unchanged)
